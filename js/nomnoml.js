    ...
    function render(graphics, config, compartment, setFont) {
        ...
        function renderCompartment(compartment, style, level) {
            ...
            compartment.lines.forEach(function (text, i) {
                ...
                if (text) {
                    // START edit by Jiaxin, to support same name but different ids
                    if (text.includes('-')) {
                        text = text.split('-')[0];
                    }
                    // END
        ...
        function renderNode(node, level) {
            ...
            g.fillStyle(config.fill[level] || style.fill || nomnoml.skanaar.last(config.fill));
            ...
            node.compartments.forEach(function (part, i) {
                ...
                // edit by Jiaxin
                var s = i > 0 ? nomnoml.buildStyle({ stroke: style.stroke, center: true }) : style;
                if (s.empty)
                    return;
                g.save();
